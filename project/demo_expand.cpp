...
int main(int argc, char *argv[]) {
  ...
  int opt;
  opt = 0;
  std::cout << "Create (0) or import key (1)? (0/1) ";
  std::cin >> opt;
  if (opt) {
    std::cerr << "Debug: Import key" << std::endl;
    void *privkey = std::malloc(party.getSizePrivkey());
    void *pubkey = std::malloc(party.getSizePubkey());
    std::string fileNameFormat;
    fileNameFormat = "privkey_";
    load_data(fileNameFormat + argv[1], privkey, party.getSizePrivkey());
    party.importPrivkey(privkey);
    fileNameFormat = "pubkey_";
    load_data(fileNameFormat + argv[1], pubkey, party.getSizePubkey());
    party.importPubkey(pubkey);
    std::free(privkey);
    std::free(pubkey);
  } else {
    std::cerr << "Debug: Create key" << std::endl;
    party.createPrivkey();
    party.createPubkey();
    void *privkey = std::malloc(party.getSizePrivkey());
    void *pubkey = std::malloc(party.getSizePubkey());
    std::string fileNameFormat;
    fileNameFormat = "privkey_";
    party.exportPrivkey(privkey);
    save_data(fileNameFormat + argv[1], privkey, party.getSizePrivkey());
    fileNameFormat = "pubkey_";
    party.exportPubkey(pubkey);
    save_data(fileNameFormat + argv[1], pubkey, party.getSizePubkey());
    std::free(privkey);
    std::free(pubkey);
  }
  // Export public key
  ...
  */
  ...
  // Create pre expand
  opt = 0;
  std::cout << "Create pre expand (again)? (0/1) ";
  std::cin >> opt;
  if (opt) {
    std::cerr << "Debug: Create pre expand" << std::endl;
    for (int i = 0; i < numParty; i++) {
      if (i == idParty)
        continue;
      void *mem_pub = std::malloc(party.getSizePubkey());
  ...
  */
  // Create ciphertexts and self-expand
  opt = 0;
  std::cout << "Create ciphertexts (again)? (0/1) ";
  std::cin >> opt;
  if (opt) {
    std::cerr << "Debug: Create ciphertexts" << std::endl;
    for (int i = 0; i < NUM_BIT; i++) {
      bool msg = (plaintext >> i) & 1;
      void *main_cipher = std::malloc(party.getSizeMainCipher());
  ...
  */
  ...
  std::cin.get();
  opt = 0;
  std::cout << "Evaluation (again)? (0/1) ";
  std::cin >> opt;
  if (opt) {
    std::cerr << "Debug: Evaluation" << std::endl;
    // Import all input cipher
    std::vector<std::vector<thesis::TrgswCipher *>> cipher_list(numParty);
    for (int i = 0; i < numParty; i++) {
      cipher_list[i].resize(NUM_BIT, nullptr);
